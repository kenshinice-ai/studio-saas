...
Production remain separate rows. **v10.1.1 is committed; packaging and deploy are in progress** as of
14 August 2026 — Production still runs v10.1.0 until the row below says otherwise.

| Layer | Verified state | Evidence |
|---|---|---|
| Source | **v10.1.1 committed** | `VERSION` = **10.1.1**; one name per page (`cmsPageTitle` derived from `NAV` — the two hand-written maps had drifted), body headings no longer duplicate the sticky header, and the English console filled in by walking the *rendered* text nodes rather than guessing from source. Migration 0040 adds `tenant_billing_identity`: an Australian tax invoice needs the issuer's ABN and there was nowhere to put it, so the money layer could not legally issue. Migration 0041 fixes a v10.0.0 bug where **any tenant with an issued invoice could not be deleted** — the immutability trigger fired on the cascade from `DELETE FROM tenants`. The showcase now seeds a term of money and is the reset baseline. pytest `2559 passed, 28 skipped`. |
| Package | **v10.1.0 built; v10.1.1 not yet packaged** | SaaS `dist/PWE-StudioSaaS-aws-10.1.0.tar.gz` SHA-256 `319a2d53c2944b1f785495c69e056398263a1a07ae689ec0177af41bd24ef15f`; Edition `dist/PWE-Studio-Edition-10.1.0.tar.gz` SHA-256 `8d65d7b29ecb3c1e9d202bf377b068ea5ad8a9a62f39139f44926380310cbe73`. Both `BUILD_INFO` records read v10.1.0 with modes `saas` / `standalone`; both passed checksum, entrypoint, version and exclusion checks via `verify_release_bundles.sh`. 25 MB each. The compiled `cms-app.js` inside the tarball was checked to contain the five panels, not merely their sources — the bundle is what runs. |
| Production | **v10.1.0 deployed; v10.1.1 not yet deployed** | Deployed 14 August 2026 from `PWE-StudioSaaS-aws-10.1.0.tar.gz` (SHA-256 `319a2d53…`). Migration `0039_plan_features_for_the_money_layer.sql` logged as `applied` by the entrypoint, and its effect was read back off the live database rather than inferred from the log: `growth` now carries `teacher_payables`/`sms_notifications`/`management_reports`, `studio` the first two, `starter` none — which is the tiering the resolver expects. Deep health reports `appVersion=10.1.0`, `db=ok`, `mode=saas`, `tenants=6`, `themes.unreadable=0`, `workspaces.stale=0`, disk free `44.0 GB`. The five new scheduling and progress-report routes answer `401` while an unknown path under the same prefix answers `404` — evidence the routes are registered, not that a tarball landed. The public CMS serves `cms-app.js?v=10.1.0` and that served file contains all five panels. Pre-deploy backup `studiosaas_studiosaas_20260814T121613Z.dump` with its manifest. |
...
`tenant_slug_aliases` as a permanent 301 and never reissued. Section 00 now says
so, section 11 explains the guarantee, and the questions section answers it —
it is a question asked before signing, and the manual had been answering "no".

**All twenty screenshots were re-taken against this release.** They had drifted
onto three different baselines (CMS at v9.5.0, Studio Admin at v9.6.1, the
showcase pair at v9.8.5) while Studio Admin changed by 1,165 lines and the
public shell was rewritten. The workbench rail had grown from ten panels to
twelve, and the manual's own annotation still said ten. A new pair,
`02-showcase-page`, covers the Selected Work page that has had an address of its
own since v9.8.10 and had never been photographed.

**`TIMETABLE_PUBLIC_SEED` is gone from the capture script.** The public
timetable shot used to be taken through a hand-written fixture, because before
v9.9.2 the showcase tenant had no timetable data. The seeder owns that half of
the tenant now, so the fixture had become a second source of truth — and it had
drifted far enough to render no booking buttons at all, which is what failed the
capture. That shot photographs the real page now, like every other shot.

**The bilingual roadshow deck was refreshed alongside it.** Twelve of its
thirteen product screenshots were copies of manual shots and were replaced from
the same set; the review-queue slide had been showing a pre-v9.5.0 console that
no longer exists.

## v9.9.5 release scope
