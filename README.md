...
Production remain separate rows. **v10.2.0 is committed; packaging and deploy are in progress** as of
14 August 2026 — Production still runs v10.1.1 until the row below says otherwise.

| Layer | Verified state | Evidence |
|---|---|---|
| Source | **v10.2.0 committed** | `VERSION` = **10.2.0**; one `<FilterBar>` shared by billing and teacher pay (date range, search, counted state chips, always-visible result count and clear); settings became a real destination with real tabs — the pills declared `role="tablist"` and called `scrollIntoView`, so nine sections rendered at once and a screen reader was promised panels that did not exist. Two bugs the wiring exposed: `per_hour` printed raw on teachers' pay sheets, and the φ master/detail grid had no breakpoint, leaving a 143px secondary column on a phone. Manual gained chapter 10 with four screenshots retaken from the seeded showcase. pytest `2562 passed, 28 skipped`. |
| Package | **v10.1.1 built; v10.2.0 not yet packaged** | SaaS `dist/PWE-StudioSaaS-aws-10.1.1.tar.gz` SHA-256 `5963113148942b4377819d43bbc919068a2175780ae110bf072e030c927e2119`; Edition `dist/PWE-Studio-Edition-10.1.1.tar.gz` SHA-256 `820deaf3782781efe7742c9b7cc7dd6a3bef2cfcdb136a8c87672161bebbd7e7`. Both passed `verify_release_bundles.sh` — checksum, BUILD_INFO, entrypoint, version and exclusion checks. |
| Production | **v10.1.1 deployed; v10.2.0 not yet deployed** | Deployed 14 August 2026 from `PWE-StudioSaaS-aws-10.1.1.tar.gz` (SHA-256 `59631131…`). Migrations `0040_tenant_billing_identity` and `0041_purge_can_delete_issued_invoices` logged as `applied`. 0041 was verified by exercising it on the live database rather than trusting the log: inside a rolled-back transaction, a tenant holding an issued invoice deletes when `studiosaas.purging` is declared and a bare `DELETE FROM invoice_lines` is still refused with the credit-note message — both halves, because a fix that disabled the guard entirely would also have passed the first check. Deep health reports `appVersion=10.1.1`, `db=ok`, `tenants=6`, `themes.unreadable=0`, `workspaces.stale=0`, disk free `44.4 GB`. The showcase tenant was reset to the v10.1.1 baseline in production: 4 payers, 5 invoices, 6 teaching sessions, 2 private lessons, 1 make-up credit, 4 progress reports, Xero add-on active with the gate deliberately unsatisfied. Pre-deploy backup `studiosaas_studiosaas_20260814T125808Z.dump`. |
