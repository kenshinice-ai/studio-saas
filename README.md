...
Production remain separate rows. **v10.5.0 is committed; packaging and deploy are in progress** as of
15 August 2026 — Production runs v10.3.0, verified by live probe rather than inferred.

| Layer | Verified state | Evidence |
|---|---|---|
| Source | **v10.5.0 committed** | `VERSION` = **10.5.0**. Four defects found by running production with real sessions, none of them introduced by v10.3.0 — git shows all four identical at v10.2.2. (1) nginx `gzip_types` omitted `text/javascript`, the type Flask serves `/assets/*.js` under, so every application script went out uncompressed — `cms-app.js` at 590KB instead of 119KB, ≈582KB per first visit. (2) The tenant header inherited `.wrap`'s 1180px reading measure, making `fitNavigation()`'s budget a constant 874px against a need of 1006px: the English nav collapsed to a hamburger at every desktop width, including 5K. (3) Contract keys of the form `#home:courses` reached `href` unchanged, so four nav entries pointed at element ids that do not exist and moved the page zero pixels. (4) `seo_title`/`seo_description` are bilingual pairs but were written through a string-only reader ending in `str(value)`, storing a Python repr that then rendered into `<title>`, `og:title` and every share card. Also carries v10.3.1's `db.use_owner_connection()`, and removes the version label from the marketing and pricing pages (the manual keeps its own, which identifies what it documents). |
...
| Production | **v10.3.0 deployed; v10.5.0 not yet deployed** | Live probe on 15 August 2026: `/v1/health` reports `appVersion=10.3.0`. Exercised with real sessions — tenant owner and super admin with an active support session — across 30 tenant routes and 23 console sections: zero 500s, zero JS errors. Tenant isolation confirmed from outside: a super admin scoped to one studio receives `403 support_session_required` on every other studio's data, while its own returns 200. The platform demo reset completed in 7.5s. The README previously recorded v10.2.2 here; that row was stale, and the correction is what a live probe is for. |
