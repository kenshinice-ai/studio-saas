...
Production remain separate rows. **v10.1.0 is committed; packaging and deploy are in progress** as of
14 August 2026 — Production still runs v10.0.0 until the row below says otherwise. That gap is
the reason these are three rows and not one label.
...
| Package | **v10.0.0 built; v10.1.0 not yet packaged** | SaaS `dist/PWE-StudioSaaS-aws-10.0.0.tar.gz` SHA-256 `7590f6fc2e6d3cd1a10d99de08f6ad2264d166cb9b2340d8629eda47397d5fff`; Edition `dist/PWE-Studio-Edition-10.0.0.tar.gz` SHA-256 `c2a4f53c7bf98359eadb86aefcf880092279e1cde94f9e809948dca8b6b90bd3`. Both `BUILD_INFO` records read v10.0.0 with modes `saas` / `standalone`; both passed checksum, entrypoint, version and exclusion checks. 25 MB each — the showcase seed sources are `export-ignore`d, because the demo reset reads only `seed-assets/showcase/`. |
| Production | **v10.0.0 deployed to `pwestudio.online`; v10.1.0 not yet deployed** | Deployed 14 August 2026 from `PWE-StudioSaaS-aws-10.0.0.tar.gz` (SHA-256 `7590f6fc…`). All seven migrations applied in order on the live database — `0032_tenant_addons` through `0038_channels_calendar_and_progress_reports`, each logged as `applied` by the entrypoint. Deep health reports `appVersion=10.0.0`, `db=ok`, `mode=saas`, `tenants=6`, `themes.unreadable=0`, `workspaces.stale=0`, disk free `44.3 GB`. The five authenticated money routes answer `401` rather than `404`, and the public calendar feed answers `404` for an unknown token rather than `500` — which is the evidence that `calendar_subscriptions` exists and is queryable, not merely that the code shipped. Pre-deploy backup `studiosaas_studiosaas_20260814T031503Z.dump` with its manifest. |
