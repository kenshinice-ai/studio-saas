...
Production remain separate rows. **v10.1.0 has completed all three milestones** on 14 August 2026; the rows
below record the independent evidence for each.

| Layer | Verified state | Evidence |
|---|---|---|
| Source | **v10.1.0 committed** | `VERSION` = **10.1.0**; the money layer grows an interface — 5 CMS panels (billing, finance, Xero wizard, progress reports, private lessons), `services/scheduling.py` and 10 scheduling routes, migration 0039 so plan rows finally know the v10.0.0 feature keys exist. Full gate green: pytest `2555 passed, 28 skipped`, legacy CMS smoke `73 passed`, tenant isolation `237 passed, 0 failed`. Fixes a 500 that shipped live in v10.0.0: `progress_reports.assemble()` put psycopg `date` objects into `json.dumps`, so any studio with lesson notes crashed on report creation. Every panel was driven end-to-end in a browser against a real tenant, not declared working from a green compile.
| Package | **v10.1.0 built and verified** | SaaS `dist/PWE-StudioSaaS-aws-10.1.0.tar.gz` SHA-256 `319a2d53c2944b1f785495c69e056398263a1a07ae689ec0177af41bd24ef15f`; Edition `dist/PWE-Studio-Edition-10.1.0.tar.gz` SHA-256 `8d65d7b29ecb3c1e9d202bf377b068ea5ad8a9a62f39139f44926380310cbe73`. Both `BUILD_INFO` records read v10.1.0 with modes `saas` / `standalone`; both passed checksum, entrypoint, version and exclusion checks via `verify_release_bundles.sh`. 25 MB each. The compiled `cms-app.js` inside the tarball was checked to contain the five panels, not merely their sources — the bundle is what runs. |
| Production | **v10.1.0 deployed to `pwestudio.online`** | Deployed 14 August 2026 from `PWE-StudioSaaS-aws-10.1.0.tar.gz` (SHA-256 `319a2d53…`). Migration `0039_plan_features_for_the_money_layer.sql` logged as `applied` by the entrypoint, and its effect was read back off the live database rather than inferred from the log: `growth` now carries `teacher_payables`/`sms_notifications`/`management_reports`, `studio` the first two, `starter` none — which is the tiering the resolver expects. Deep health reports `appVersion=10.1.0`, `db=ok`, `mode=saas`, `tenants=6`, `themes.unreadable=0`, `workspaces.stale=0`, disk free `44.0 GB`. The five new scheduling and progress-report routes answer `401` while an unknown path under the same prefix answers `404` — evidence the routes are registered, not that a tarball landed. The public CMS serves `cms-app.js?v=10.1.0` and that served file contains all five panels. Pre-deploy backup `studiosaas_studiosaas_20260814T121613Z.dump` with its manifest. |
