...
Schema 文件：`backend/db/schema_v1.sql`
...
5. 前端入口：拆出 Super Admin 和 Studio Admin 两个入口占位。
6. 权限与审计：定义角色、权限检查、审计记录接口。
7. 迁移脚本：把现有 `database.json` 导入为第一个 tenant。
8. 套餐限制：先定义套餐和用量边界，后续接入 Stripe 或其他支付。

当前运行目录策略：`backend/` 是正式后端目录；`legacy-root/` 保留为
租户化桥接层，用来承载旧 CMS/Register UI 并把旧 `/api/*` 请求改写到
tenant-scoped PostgreSQL API。
