# ✅ Platform Admin 优化清单（我的排序，2026-08-08）

写在 `docs/design/Platform_Admin_Workspace.md` 顶部。**仍未改代码。**

**核心判断：三栏 shell 不该排第一。** 线上 6 个租户，当下的问题不是
「列表与详情来回太多」。而盘点中发现的**三条问题是当前产品里就有的**，
换个外壳不会消失。

## 三条新发现（已对代码核实）

1. **界面声称四个操作都有确认短语，实际只有两个有。**
   `super-admin.html:3141` 写着 pause / archive / restore / permanent delete
   「each one asks for its own confirmation phrase」。实测：
   permanent delete ✅、archive ✅、**pause ❌**、restore ❌。
   **暂停会把整间工作室锁在门外**，而界面告诉操作者有一道不存在的闸门。
2. **支持会话进行中没有任何全局提示。** `enterSupportMode()` 存在，但外壳上
   看不出「你现在正在某个租户里」。**租户 Owner 在自己的审计面板里看得见，
   平台方自己反而看不见。**
3. **`showcase_limit` 用量在任何界面都看不到**（`grep showcase super-admin.html`
   = 0）。它是套餐差异化的卖点（15/60/150）——客户问「我该不该升级」，
   平台方答不上来。

## 批次

- **第 0 批（半天，不碰布局，可今天发）**：补 pause 的确认短语（或把那句话改成
  实话）· 支持会话常驻横幅 · 文案修正 + 左栏裁掉四个不存在的功能
- **第 1 批（一天）**：四条真实配额（含 showcase）· 系统健康接概览
  （数据已在 `/v1/health?deep=1`，且是唯一能提前发现故障的一格）· 审计前后值
- **第 2 批（三到四天）**：三栏 shell，按评审那三条改。
  **现有的 `DELETE <slug>` 键入闸门必须保留 —— 它已经是对的，别在重构里弄丢**
- **第 3 批（一天）**：状态统一，含渲染稿没有的 `partial`

## 建议不做

多选比较 · 批量操作 · 保存筛选（6 个租户都没有这些问题，写进路线图 =
永远排不上的债）· 邀请/公告/分组/发票（四个从没排期的子系统）·
平台级存储总配额（这个概念不存在，别为填一个进度条把它造出来）

## 顺序的理由

第 0 + 第 1 批合计一天半、不碰布局，修的是「说了假话」和「有数据不给看」。
**三栏 shell 排第三不是因为它不好，是因为它此刻解决的问题最小** ——
到 50+ 租户它会变第一位，那时也更清楚 Inspector 里该放什么。
