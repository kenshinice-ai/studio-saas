...
1. 约课要不要限制提前天数？倾向要（默认 = `timetable_weeks` 的范围），
   否则会收到三个月后的请求。
2. 同一手机号对同一节课重复提交？倾向：已有 pending 就返回「已经收到了」，
   不新建。
