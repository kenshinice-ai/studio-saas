...
> **状态更正（2026-08-23）：**本文列出的 Platform Admin 三栏 shell、支持模式
> 全局提示、套餐/作品用量和操作确认等工作已在 v9.7.0–v9.8.5 分阶段落地。
> 保留本文作为优先级与根因记录，不再把“当前产品里就有的问题”当作当前缺陷。

问「你认为合适的」，所以这一节是我的判断，不是把你的 P0/P1/P2 换个标签。

**核心判断：三栏 shell 不该排第一。** 线上 6 个租户，控制台当下的问题不是
「列表与详情之间来回太多」。真正值钱的在别处 —— 而且其中三条是**当前产品里
就有的问题**，跟新外壳没关系。

先说三条我在盘点时新发现的（都已对代码核实）：

| 发现 | 严重度 |
|---|---|
| **界面声称四个操作都有确认短语，实际只有两个有** | 高 |
| **支持会话进行中，没有任何全局提示** | 高 |
| **`showcase_limit` 用量在任何界面都看不到** | 中 |

---

## 第 0 批 · 界面说了但产品没做（半天，先发）

**A0-1 · 修掉一句假的安全承诺（最高优先）**

`super-admin.html:3141` 写着：

> "Pause, archive, restore and permanent delete live in the More actions menu,
> **where each one asks for its own confirmation phrase**."

实测：`permanentDeleteTenant` ✅ 有、`archiveTenant` ✅ 有、
**`pauseTenant` ❌ 没有**、`restoreTenant` ❌ 没有（restore 无所谓，它不破坏）。

**暂停会把整间工作室锁在门外**，而界面告诉操作者「有一道短语闸门」——
**那道闸门不存在**。两条路选一条：给 pause 补上确认短语，或者把这句话改成
实话。我建议**补上**：暂停的代价和归档同级。

**A0-2 · 支持会话要有全局可见状态**

`enterSupportMode()` 存在，但外壳上**没有任何东西显示「你现在正在某个租户里」**。
这是全系统权限最高的状态，而它是隐形的。

一条常驻横幅：`支持会话进行中 · <租户名> · 已持续 12 分钟 · [结束会话]`。
租户 Owner 在自己的审计面板里看得见这件事 —— **平台方自己反而看不见，说不过去**。

**A0-3 · 文案修正 + 左栏裁剪**

`Payment failed` → `Subscription past due`；`MRR (USD)` → `AUD`；
`Impersonate login` → `Start support session`；
`Reset admin password` → `Send password setup link`；
`Plan expiring soon` → `Trials ending`；
删掉 `Bandwidth` / `Projects` / `Invoices` / `Invitations` / `Announcements` /
`Tenant Groups`。

> 这一批**不碰布局**，可以今天发。它止住的是「界面承诺了产品做不到的事」——
> 一个平台方读到 "Invoices" 会去找发票，读到确认短语会以为暂停有闸门。

---

## 第 1 批 · 把已有的数据露出来（一天）

**A1-1 · Inspector / 租户详情显示四条真实配额**

学员 · 员工 · 存储 · **作品发布上限**。前三条数据已在 `tenant_usage`，
第四条在 `plans.showcase_limit`。

**`showcase_limit` 尤其要加**：它是套餐差异化的卖点（15/60/150），
而平台方现在**在任何界面都看不到某个租户用了多少** —— 客户打电话问
「我是不是该升级」，平台方答不上来。

**A1-2 · 系统健康接到概览**

数据已经全在 `/v1/health?deep=1`（`db`、`disk`、`themes.unreadable`）。
这是**唯一一格能提前发现故障**的东西 —— v8.5.4 那次五个门户空白，
正是 `themes.unreadable` 这个指标存在的原因。接上它比新建任何东西都便宜。

**A1-3 · 审计事件的前后值对比**

`/admin/audit-logs` 的 metadata 里已经有内容。平台方复核退款和配额调整时
最想要的就是这个，成本很低。
...
到这一步再动布局。我同意这个方向，但按前面评审里那三条改：

1. **Inspector 可折叠 / 固定**，`<1440px` 默认折叠（6 个租户时那 380px 是负担）
2. **选中项变化即关闭 Support Mode** 并提示
3. **破坏性操作不进 Inspector** —— 走确认框，标题带租户名 + slug。
   **现有的 `DELETE <slug>` 键入闸门必须保留**（它已经是对的，别在重构里弄丢）
4. 中栏 / 右栏独立滚动；Inspector 选中项进 hash（否则刷新即丢）
5. 标签跟随语言切换器，**只有破坏性操作的确认文案保持双语**

## 第 3 批 · 状态与验收（一天）

loading / empty / **partial** / error / retry / success 统一。
`partial` 是渲染稿没有的那一个：`/admin/usage` 是一条大 SQL，
某个子查询慢或失败时，**已拿到的数字要照常显示**，不能整块转圈。

`Attention Queue` 的多租户告警 → 进**中栏筛选列表**，不进右栏
（Inspector 一次只装一个租户，渲染稿没解决这个映射）。
...
| 项 | 为什么 |
|---|---|
| 多选比较租户 / 套餐 | 6 个租户没有这个问题 |
| 批量低风险操作 | 同上。现在写进路线图 = 永远排不上的债 |
| 保存筛选视图 | 6 个租户不需要保存筛选 |
| 邀请 / 公告 / 分组 / 发票 | 四个从没排期的子系统。**先确认真的要，再排** |
| 平台级存储总配额 | 这个概念不存在，别为了填一个进度条把它造出来 |

---

## 顺序的理由

**第 0 批和第 1 批加起来一天半，且不碰布局** —— 它们修的是「说了假话」和
「有数据不给看」，这两类问题在任何布局下都存在，换个外壳不会消失。

**三栏 shell 排第三，不是因为它不好，是因为它此刻解决的问题最小。**
等租户到 50+，它会变成第一位 —— 那时候再做，而且那时候你会更清楚
Inspector 里到底该放什么。
