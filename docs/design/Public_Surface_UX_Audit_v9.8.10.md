...
- **扫码进 `/<slug>/register` 的家长，没有任何导航可以去别处。** 报名页正是二维码、传单、投放的落地页，
  它同时是转化终点和入口——现在只有终点。
...
- 把 header / footer 提成**一份**公共 shell 片段。tenant-template 是静态 HTML 且刻意不引框架，
  所以做法是：模板里放 `<div data-public-shell="header">` / `="footer"` 占位，
  `public-surface.js` 按契约注入同一套结构与 id。四页从此不可能再分叉。
- 报名页补齐导航与移动菜单，店名 id 统一为 `footName`。
- 加测试：四个模板的 `nav*` / `mnav*` / `foot*` id 集合必须一致；
  `public-surface.js` 的遮罩选择器与 ids 映射中的每个 id，必须至少在一个模板里存在。
...
1. **拆字段**：导航标签与版块标题分离——`nav_label` 缺省回退到 `section_label`，
   店主想改导航文字就单独改。或者退一步：至少在 Studio Admin 明确标注「这也是导航文字」，
   并给出字数上限（中文 6 字、英文 16 字为安全值）与实时导航预览。
2. **`navPrimaryCta` 给横向 padding**，`white-space: nowrap`，超长省略号。
3. **导航自身要能降级**：项目数或总宽超过容器时收进「更多 ▾」。不要指望店主自觉——
   Ruby 的数据说明他们不会。
