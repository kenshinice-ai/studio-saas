...
## v10.20.1 — 为新主机补上带守卫的发布路径

**只改基础设施，产品没有任何变化。**

生产环境已于 2026-09-17 迁到新硬件。本版为它补上带守卫的发布路径：一次部署必须
先证明它连的就是工作室官网实际运行的那台机器（在改动任何东西**之前**），之后还必须
证明线上确实在跑刚发布的版本。旧机器仍保留作回滚用，而它仍在运行——这正是守卫要
防的那件事：对着旧机器部署会全程「成功」，却不改变任何人看到的东西。

**零迁移，零运行时代码改动，工作室这边无需操作。**

**验收证据**：`docs/handoff/claude/2026-09-17-oracle-deploy-path.md`。

## v10.20.0 — 安静一点的后台：一页一个主操作，一个数字只说一遍

**对使用者可见的部分**

- **学员卡片的三个动作分了主次。** 打开档案在前，充值与排课在后。此前一整列十二个
  同等急迫的实心按钮，最响的那个还是「把人加进今天的排课」——而你来这一页通常是为了
  **找人**。
- **签到名单保留每行的签到键**（人来一个按一个就是这件事本身），而会把所有人一起签掉
  的批量签到不再压着它们喊。
- **「待上课」只在不是常态时才出现。** 于是签一个人进去，那一行看得见地变了；课时不足
  成了名单里唯一的记号。
- **一个数字只说一遍。** 今天的人数过去在工作台印三遍、学员总数印两遍；课表的列表标题
  也不再重复它正上方的日期。
- **后台不再有任何小于 11px 的文字。** 课时徽章会告诉读屏软件它数的是什么。
- **侧栏那个绿色「已连接」删掉了**——它没接任何状态，断线了照样说已连接。顶栏那个是
  真的，留着。
- **平台控制台**：租户表不再把与上方标签一模一样的订阅状态再印一遍；页头的 Refresh
  不再是全页最响的控件。
- **客户发布说明页在手机上不再横向滚动。**

**不改任何数据。** 零迁移，工作室这边无需操作。

**验收证据**：`docs/handoff/claude/2026-09-12-back-office-hierarchy.md`。

## v10.19.0 — 像样的错误页、一个联系邮箱、一个真能发出去的咨询表单

**对使用者可见的部分**

- **输错地址给的是一页说明，不是一行 JSON。** 双语，按地址决定语序，写清楚工作室的
  公开地址长什么样，并给出回首页与看手册两个去处。程序读的接口仍然返回 JSON。
- **一个对外邮箱。** 隐私政策、服务条款、支持政策统一为 `info@pwestudio.site`。
  此前三份文件用的是一个私人 Gmail，`/pricing.md` 又是第三个地址，而那个域名没有
  收信记录。
- **咨询表单现在会填收件人。** 两个按钮此前打开的都是一封没有收件人的空邮件；中文
  内容稍长时更是什么都不弹（邮件地址栏有长度上限，中文编码后一个字占九个字符）。
  现在会量出来再截断，并在正文里说明截断了。
- **工作室官网在脚本出错时也能读。** 此前一处报错会让整页几乎全部不显示。
- **无障碍**：手册的步骤编号与搜索高亮、两个控制台的二十处状态文字与六处悬停状态、
  三处缺失的键盘焦点框、次级按钮的描边，都达到文字与控件的对比度标准；「减少动态」
  下 CMS 的「连接中」指示又能动了；毛玻璃界面在「降低透明度」下有实底兜底。
- **产品站**：主按钮改为墨色，琥珀色只保留在收尾的联系区一处，推荐套餐带上主按钮。
  定价页的价格只来自套餐表。

**不改任何数据。** 零迁移，工作室这边无需操作。

**验收证据**：`docs/handoff/claude/2026-09-12-brand-realignment-r1.md`。
...
`docs/design/Consolidated_Improvement_Plan_2026-09-06.md` 附录 D。

这一版做的是 v10.16.0 明写「留到下一轮」的四件事。做的时候发现其中两件的病因
和记录的不一样，而顺着它们又挖出四个没人记过的缺陷 —— 三个在门禁自己身上。

**你会看见的**

- **学员注册页快了。** `/_legacy/register` 曾经在打开时下载一个 451KB 的 CSS
  编译器并在浏览器里现场编译；现在是一份 17KB 的静态样式表。**外观逐像素不变**
  —— 改动前后比对了 166 个元素的 21 个计算属性，零差异。
- **发给家长的成长报告，图标终于画出来了。** 那五个图标此前是一段被误当成组件
  写进 HTML 的标记，浏览器画零像素，还会把按钮上的文字吞进去。同一段里四句本该
  跟着报告语言走的文案（复制寄语、打印、已复制、暂无作品记录）从来没接上，英文
  工作室发出去的报告夹着中文 —— 现在整份报告只说一种语言。
- **侧栏里的状态文字看得清了。** 绿色和红色的状态标签此前在侧栏和课程帮助卡里
  对比度只有 2.75:1 和 3.74:1（正文里是 7.41 和 10.48）。现在是 8.50 和 12.02。

**内部**

- 本机发布门禁从「红了十一个版本」变成全绿：它此前用超级用户跑应用侧检查，而
  超级用户绕过行级安全，那两条租户隔离测试永远不可能通过。
- 浏览器回归矩阵不再把「没检查」报成「0 失败」；生产实测 416 条断言、
  12/13 个页面。
- `npm ci` 修好了（v10.16.0 起锁文件与 package.json 不同步）。发出去的字节没有
  问题，坏的是可复现性。

## v10.16.0 — 两份审计的交叉核实，与它们都没找到的那一条

验收证据见 `docs/HANDOFF_LATEST.md` 的四层身份表，方案与实测记录见
`docs/design/Consolidated_Improvement_Plan_2026-09-06.md`。

这一版从两份同日、方法不同的独立审计开始，逐条核实之后修的东西。

**最重要的一条，两份审计都没有发现：** 「一对一循环课与补课额度」里点「安排补课」，
会把家长owed的那次补课额度不可逆地消耗掉、**不在课表上排出任何一节课**，并弹出
绿色的「补课已登记」。家长按约定日期来了，这节课不在任何人的名单上。现在这两件事
是一个事务的两半，排不出课就不会扣额度。

**其它会影响到你每天工作的修复：**

- 点「已联系」不再清空已经设好的下次跟进时间。待处理页现在是一个可以按
  「今天需跟进 / 未联系 / 已约试听」筛选、按姓名手机号搜索的队列，到期的排在前面。
- 结算后点「查看发票」会打开那张发票，而不是一个显示「还没有发票」的空账本。
- 手机上的「网站与品牌」终于可以编辑：编辑区曾经被预览挤到只剩 2px 宽。
- CMS 出现程序错误时会显示一张说明卡片和一个返回工作台的按钮，而不是一片空白；
  登录过期不再把你填到一半的内容清掉，改为原地重新登录。
- 经营月报的营收、签到与新生数改为在服务端全量计算 —— 之前它只统计最近 500 条
  记录，对忙碌的工作室来说这意味着「近六个月」实际只覆盖了最近两三周。
- 手机底部导航终于有图标；工作台上同一个去处不再出现两三次；全新工作室会看到一张
  四步开通清单，做完自动消失。
- 后台首次打开时会跟随浏览器语言，不再一律中文；涉及金额与不可撤销操作的提示
  补齐了英文。
- 官网在部分内容加载不出来时，只显示一条会自动重试的提示，而不是五条盖住首屏的
  英文报错；从课程卡片点报名，表单会记得你问的是哪门课。
- CMS 打开速度：浏览器不再需要先下载并运行一个 451KB 的样式编译器。

**顺带修好的一处老问题：** 后台所有弹窗自 v8.4.2 起都没有背景遮罩层，现在有了。

## v10.15.0 — 后台不再层层堆叠，官网多了三层材质

（这一节是 v10.16.0 发布时补写的：上一轮的骨架未填就发布了，
这里按当时的实测事实补齐，未改动任何已发生的事实。）

- 工作台四条同类提醒条合并成一个「需要注意」区，`最近操作` 改为一行入口：
  落地页桌面高度 −43%、手机 −46%。
- 课程安排分成「今日签到 / 排课设置」两个标签；对老师与助教，可见按钮从 55 降到
  26。新增整月展开，与派生的「未签到」状态。
- 设置页在 375px 手机上七个分区全部可见（此前后四个被裁到视口外）。
- 官网深色带加了纸纹、课程卡片加了分类水印、长页面顶部加了一条阅读进度线；
  首屏轮廓形状跟随工作室的视觉风格。
