...
> 适用版本：StudioSaaS v7.4.1 · 界面：Super Admin 控制台（`/super-admin`）
...
Surfaces（Portal / CMS / Register / Admin 四个直达链接；归档或删除的租户
链接置灰）、Actions（**View** 与 **More**）。
...
## 四、支持模式（Support Session）

帮客户排障时使用，让你的操作带上审计标记：
...
> **重要：支持模式是 UI 提示与审计标记，不是强制权限门。** Super Admin
> 不开支持模式也能访问租户路由；开启的意义是把这段时间的操作都打上
> `support_session` 审计标记（Audit Logs 里对应 `support.session_started` /
> `support.session_ended`）。平台内部规范应要求：凡进入租户界面必先开
> 支持会话。
...
**Q7：不开支持模式直接进租户后台有技术限制吗？**
没有——这正是要靠流程约束的地方。请始终先开支持会话，让操作可追溯。
...
| 平台审计日志 | ✅ | 无筛选/导出 |
| 支持模式进入租户 Studio Admin / CMS | ✅ | 技术上不强制，流程上必须填 Reason |
| 租户内所有 API（权限 `*`） | ✅ | 仅限支持场景使用 |
| 数据库备份 / 恢复 | ✅（脚本） | 控制台无入口，见 Admin_Guide |
| 替租户日常运营 | ⚠️ 不建议 | 属 Owner/Manager 职责 |

---
相关手册：[Owner 手册](Studio_Owner_Guide.md) · [Manager 手册](CMS_Manager_Guide.md) · [手册总览](README.md)
