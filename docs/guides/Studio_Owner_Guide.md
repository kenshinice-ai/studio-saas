...
> 适用版本：PWE Studio v8.2.28 · 界面：Studio Admin（`/<工作室网址标识>/studio-admin`）
> 与运营 CMS（`/<工作室网址标识>/cms`）
...
- 选择主题下方有预览卡：主题名、推荐/已选择/自定义徽章、色相关系，以及
  **两组色板**：
  ...
  一个普通按钮）。现在 45 个状态色是**按主题逐一求解**的——每个都同时满足
  文字对比度 ≥ 4.5:1、填充对比度 ≥ 3:1，并与该主题的强调色保持足够色相差。
  所以换主题时状态色会跟着变，这是正确行为。
