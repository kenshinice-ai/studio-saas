...
> 适用版本：PWE Studio v8.10.3 · 界面：Studio Admin（`/<工作室网址标识>/studio-admin`）
> 与运营 CMS（`/<工作室网址标识>/cms`）
...
### 3c.1. 课程管理（CMS → 设置 → 课程管理，v8.10.3）

课程是排课可以引用的独立资料，不是必须存在的前置条件。Owner 或 Manager 在
CMS 系统设置里维护课程的**名称、简介、适龄段、时长和价格**；名称必填，后四项
选填。排课编辑器的「关联课程」下拉旁边有「去添加课程 →」或「管理课程」链接，
会直接打开这里。

课程被班次或账目引用后，移除操作是**归档而不是删除**。确认框会显示当前有几
个班次使用它；已保存的班次、账目和历史记录不受影响，归档课程只是不再出现在
新排课的下拉列表中。
