...
> 适用版本：PWE Studio v8.5.1 · 界面：Studio Admin（`/<工作室网址标识>/studio-admin`）
> 与运营 CMS（`/<工作室网址标识>/cms`）
...
- **01 选择行业预设**：点行业卡片会把该行业的推荐文案、报名问题和 FAQ
  ...
  **v8.5.0 起行业不再改配色**——它带来的是词汇和表单，颜色由你自己定。
- **02 定强调色**：见下文「主题系统」。
...
## 二、主题系统（一套配色 × 明暗 + 一个强调色旋钮）

- 从 v8.5.0 起**只有一套配色**：暖纸、近黑暖墨，加一支你自己定的强调色。
  此前是 8 套按行业分的配色，取消的原因是它让纸带上了行业色相——而哪个
  状态色跟纸同色相，那个状态色就看不见了。7 套里 5 套中招：绿色主题显示
  不出「成功」，蓝色主题显示不出「提示」。
- **Appearance**：Light / Dark，成对设计、都过了对比度校验。
- **强调色**：唯一可调的颜色，可以直接从你的 Logo 取。系统**只取它的色相**，
  明度和饱和度由系统重新求解——所以一个荧光色的 Logo 会变成一支体面的
  深色，做不出难看的站，也不会出现看不清的按钮。
- 选择下方有预览卡：**两组色板**：
  ...
  的固定值，套到某些配色上会出现两个问题：填充色与面板的对比度掉到 3:1
  以下（看不清），或者警示色与强调色太接近（一个警示徽章长得像一个普通
  按钮）。现在 6 个状态色是**逐一求解**的——每个都同时满足文字对比度
  ≥ 4.5:1、填充对比度 ≥ 3:1，并与强调色保持足够距离。状态色本身不随
  强调色变色相（红就是红、绿就是绿），但它的深浅会跟着模式走。
- **Fine-tune selected theme（微调）**：按钮风格（Soft/Sharp/Rounded）、
  字体气质，以及高级颜色（页面背景/面板背景/文字/次要文字/边框）。
  动过任何颜色后主题变为「Custom 自定义」，此时强调色旋钮不再接管——
  你自己负责对比度，发布前的门槛照样会拦。
  **一般不需要动这里**：改强调色就够了，其余是解出来的。
- 改强调色立即生效在草稿和预览上，「发布前不会影响公开页面」。
