...
（slug），系统有五个主要界面，另有一个可直接转发的公开课表页面：
...
| **Public Timetable** 公开课表 | `/<slug>/timetable` | 访客 / 学员家长 | 查看公开班次、余位、停课原因，免注册提交约课 |
...
## 一点五、v8.6–v8.10 的新功能在哪里

这些功能已经在产品里，按角色查对应的手册；它们不是“以后会有”的方案：

| 功能 | Owner | Manager | Teacher | Front Desk | 学员/家长 | 说明 |
|---|---|---|---|---|---|---|
| 课程管理 | 配置 | 配置 | — | — | — | CMS → 设置 → 课程管理；归档不删除历史引用 |
| 公开课表 | 配置 | 查看/运营 | 查看 | 查看公开页 | 查看 | `/<slug>/timetable`；每个班次逐条公开 |
| 免注册约课 | 批准/婉拒 | 批准/婉拒 | 查看公开页 | **看得到，转交 Owner/Manager** | 提交申请 | 申请不占座，批准时复核容量 |
| 停课 | 管理 | 管理 | 查看 | 查看公开页 | 查看 | 只取消某一个日期，不删除周期班次 |
| 老师公开署名 | 逐人设置 | — | 同意/撤回 | — | — | 个人同意优先于课表字段开关 |
| 余位与候补 | 配置容量 | 查看 | 查看 | 查看公开页 | 查看 | 绿 / 琥珀 / 灰三种文字状态 |
| 工作室作品 | 管理 | — | — | — | 查看 | 套餐上限 15 / 60 / 150，降级不删作品 |

约课申请在 CMS 的「待审核」页里与新报名分成两个标签。前台已有报名审批权限，
但当前**没有约课批准/婉拒权限**；前台看到申请后应转交 Owner 或 Manager，权限暂不改变。
...
- 想建课程、排公开班次、审批约课 → [Manager 手册](CMS_Manager_Guide.md)
- 想办充值、退款、审批报名 → [Manager 手册](CMS_Manager_Guide.md)
- 前台审批报名、发访问码、看到「疑似重复」角标；约课申请转交 Owner/Manager →
  [前台/员工手册](Front_Desk_Staff_Guide.md)
- 想给学员签到、传作品、读公开课表余位 → [Teacher 手册](Teacher_Guide.md)
- 家长问「还剩几节课怎么查」或「怎么约课」 → [学员/家长手册](Student_Parent_Guide.md)
