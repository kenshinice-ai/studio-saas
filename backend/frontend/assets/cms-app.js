(() => {
  ...
  function StatusChip({ invoice }) {
    ...
    return /* @__PURE__ */ React.createElement("span", { className: `text-[10px] font-bold px-1.5 py-0.5 rounded border whitespace-nowrap ${cls}` }, label);
  }
  ...
  function BillingPanel({ api, showToast, canIssue, canTakePayment }) {
    ...
    return /* @__PURE__ */ React.createElement("div", { className: "space-y-3" }, /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-2 md:grid-cols-4 gap-3" }, /* @__PURE__ */ React.createElement(Kpi, { label: "已开票", value: centsToAud(summary.billed), sub: `${invoices.length} 张 · 含 GST` }), /* @__PURE__ */ React.createElement(
      Kpi,
      {
        label: "已收到",
        value: centsToAud(summary.paid),
        tone: "good",
      ...
      "登记收款"
    ), detail.invoice.status !== "draft" && /* @__PURE__ */ React.createElement("span", { className: "text-[11px] text-gray-500" }, "已开具的发票不可修改，改错请开贷记单冲销后重开。")))), /* @__PURE__ */ React.createElement("div", { className: "bg-white border border-gray-200 rounded-xl overflow-hidden" }, /* @__PURE__ */ React.createElement("div", { className: "px-4 py-3 border-b border-gray-200 text-xs font-bold" }, "这张单发生过什么"), (detail.events || []).length === 0 ? /* @__PURE__ */ React.createElement("p", { className: "px-4 py-4 text-[11px] text-gray-500" }, "还没有记录。开具、送达、收款、推送 Xero 都会出现在这里。") : (detail.events || []).map((event, i) => /* @__PURE__ */ React.createElement("div", { key: i, className: "flex items-center gap-2 px-4 py-2 border-b border-gray-100 last:border-0 text-[11px]" }, /* @__PURE__ */ React.createElement("span", { className: "font-bold" }, event.event_type), /* @__PURE__ */ React.createElement("span", { className: "ml-auto text-gray-500" }, fmtApiDate(event.occurred_at)))))))));
  }

  // legacy-root/src/cms-app.jsx
  var { useState: useState2, useEffect: useEffect2, useMemo: useMemo2, useRef, useCallback: useCallback2 } = React;
  ...
  var readCmsRoute = () => {
    ...
    return {
      ...
      pendingTab: params.get("type") === "booking" || params.get("type") === "bookings" ? "bookings" : "registrations",
  ...
  function TenantBrandLogo({ className = "" }) {
    const [brand, setBrand] = useState2(() => window.STUDIOSAAS_BRAND || {});
    useEffect2(() => {
  ...
  function MaintSection({ onRestored, renewTh, saveRenewTh, confirm, notify }) {
    const [hc, setHc] = useState2(null);
    const [hcBusy, setHcBusy] = useState2(false);
    const [cfg, setCfg] = useState2(null);
    const [pw, setPw] = useState2("");
    const [cfgMsg, setCfgMsg] = useState2(null);
    const say = (text, tone = "info") => setCfgMsg(text ? { text, tone } : null);
    const [bks, setBks] = useState2(null);
    const [bkSel, setBkSel] = useState2(null);
...
当前数据会先自动另存为 pre_restore 备份（可再恢复回来），然后被该备份覆盖。`,
  ...
  function LoginScreen({ onLogin }) {
    const [email, setEmail] = useState2(() => localStorage.getItem(`lp_admin_email_${tenantSlug}`) || "");
    const [pw, setPw] = useState2("");
    const [busy, setBusy] = useState2(false);
  ...
  function App() {
    const [db, setDb] = useState2({ students: [], logs: [], rosters: {}, pending: [] });
    const [auditEvents, setAuditEvents] = useState2([]);
    const initialCmsRoute = useMemo2(() => readCmsRoute(), []);
    const [tab, setTabState] = useState2(initialCmsRoute.tab);
    const [pendingTab, setPendingTabState] = useState2(initialCmsRoute.pendingTab);
    const [settingsSection, setSettingsSectionState] = useState2(initialCmsRoute.settingsSection);
    const [routeRecordId, setRouteRecordId] = useState2(initialCmsRoute.recordId);
    const [moreOpen, setMoreOpen] = useState2(false);
    const [selS, setSelS] = useState2(null);
    const [editP, setEditP] = useState2(false);
    const [studentProfileTab, setStudentProfileTab] = useState2("profile");
    const [busy, setBusy] = useState2(false);
    const [conn, setConn] = useState2(false);
    const [connErr, setConnErr] = useState2(null);
    const [toast, setToast] = useState2(null);
    const [cmsNotifications, setCmsNotifications] = useState2([]);
    const [cmsNotificationUnreadCount, setCmsNotificationUnreadCount] = useState2(0);
    const [cmsNotificationOpen, setCmsNotificationOpen] = useState2(false);
    const [cmsNotificationError, setCmsNotificationError] = useState2("");
    ...
    const [showSettings, setShowSettings] = useState2(initialCmsRoute.tab === "settings");
    const [userMenuOpen, setUserMenuOpen] = useState2(false);
    const [loggedIn, setLoggedIn] = useState2(false);
    const [pwOld, setPwOld] = useState2("");
    const [pwNew1, setPwNew1] = useState2("");
    const [pwNew2, setPwNew2] = useState2("");
    const [pwBusy, setPwBusy] = useState2(false);
    const [pwMsg, setPwMsg] = useState2(null);
    const [gOpen, setGOpen] = useState2(false);
    const [gQ, setGQ] = useState2("");
    const [portLB, setPortLB] = useState2(null);
    const [portUpload, setPortUpload] = useState2(false);
    const [portUpFile, setPortUpFile] = useState2(null);
    const [portEdit, setPortEdit] = useState2(null);
    const [portBusy, setPortBusy] = useState2(false);
    ...
    const [accessCodeResult, setAccessCodeResult] = useState2(null);
    const [consentEdit, setConsentEdit] = useState2(null);
    useEffect2(() => {
    ...
    const syncCmsRoute = useCallback2((patch = {}, replace = false) => {
    ...
    const setTab = useCallback2((nextTab, options = {}) => {
    ...
    const setPendingTab = useCallback2((nextPendingTab) => {
    ...
    const setSettingsSection = useCallback2((nextSection) => {
    ...
    const [inactiveDays, setInactiveDays] = useState2(() => parseInt(localStorage.getItem("lp_inactive_days") || "90", 10));
    ...
    const [srch, setSrch] = useState2("");
    const [sortBy, setSortBy] = useState2("date-desc");
    const [filterBy, setFilterBy] = useState2("all");
    const STUDENTS_PER_PAGE = 24;
    const [studentPage, setStudentPage] = useState2(1);
    const [selectedStudentIds, setSelectedStudentIds] = useState2([]);
    const [rDate, setRDate] = useState2(todayISO);
    const [rPick, setRPick] = useState2(null);
    const [defaultClassTime, setDefaultClassTime] = useState2("14:30");
    const [defaultClassTimeDraft, setDefaultClassTimeDraft] = useState2("14:30");
    const [operationalSettingsBusy, setOperationalSettingsBusy] = useState2(false);
    const [rTime, setRTime] = useState2("14:30");
    const [icsPreview, setIcsPreview] = useState2(null);
    const [icsNotice, setIcsNotice] = useState2("");
    const [icsBusy, setIcsBusy] = useState2(false);
    const icsDialogRef = useRef(null);
    const icsCloseButtonRef = useRef(null);
    const [rOneToOne, setROneToOne] = useState2(false);
    const [grpSel, setGrpSel] = useState2("");
    const [schedules, setSchedules] = useState2([]);
    const [scheduleLoadError, setScheduleLoadError] = useState2("");
    const [bizStats, setBizStats] = useState2(null);
    const [attHistory, setAttHistory] = useState2(null);
    const [schedEdit, setSchedEdit] = useState2(null);
    const [schedPick, setSchedPick] = useState2(null);
    const [courses, setCourses] = useState2([]);
    const [schedCancel, setSchedCancel] = useState2(null);
    const [bookings, setBookings] = useState2([]);
    const [courseEdit, setCourseEdit] = useState2(null);
    const [renewTh, setRenewTh] = useState2(() => parseInt(localStorage.getItem("lp_renew_threshold") || "2", 10));
    const saveRenewTh = (v) => {
      const n = parseInt(v, 10);
      if (n >= 0) {
        setRenewTh(n);
    ...
    const [tuStu, setTuStu] = useState2(null);
    const [settleMode, setSettleMode] = useState2("topup");
    const [rfCr, setRfCr] = useState2("");
    const [rfAmt, setRfAmt] = useState2("");
    const [rfReason, setRfReason] = useState2("");
    const [tuCr, setTuCr] = useState2("");
    const [tuFee, setTuFee] = useState2("");
    const [tuPkg, setTuPkg] = useState2("");
    const [tuPay, setTuPay] = useState2("微信");
    const [lSrch, setLSrch] = useState2("");
    const [lStu, setLStu] = useState2(null);
    const [lAct, setLAct] = useState2("");
    const [lDateFrom, setLDateFrom] = useState2("");
    const [lDateTo, setLDateTo] = useState2("");
    const [lPage, setLPage] = useState2(1);
    const LPP = 30;
    const [sPeriod, setSPeriod] = useState2("monthly");
    const [sYear, setSYear] = useState2(String((/* @__PURE__ */ new Date()).getFullYear()));
    const [sFrom, setSFrom] = useState2("");
    const [sTo, setSTo] = useState2("");
    const [sStu, setSStu] = useState2(null);
    const [sStu2, setSStu2] = useState2(null);
    const [approveCredits, setApproveCredits] = useState2({});
    const [followUpDates, setFollowUpDates] = useState2({});
    const [pkgEditId, setPkgEditId] = useState2(null);
    const [pkgName, setPkgName] = useState2("");
    const [pkgCredits, setPkgCredits] = useState2("");
    const [pkgPrice, setPkgPrice] = useState2("");
    const [tenantBrand, setTenantBrand] = useState2(() => window.STUDIOSAAS_BRAND || {});
    const [team, setTeam] = useState2([]);
    const [teamBusy, setTeamBusy] = useState2(false);
    const [teamForm, setTeamForm] = useState2({ fullName: "", email: "", role: "teacher", temporaryPassword: "" });
    const [actorRole, setActorRole] = useState2("");
    ...
    const [formPhoto, setFormPhoto] = useState2("");
    const [editPhoto, setEditPhoto] = useState2("");
    ...
    const upcomingBirthdays = useMemo2(() => {
    ...
    const selectedStudents = useMemo2(
    ...
    const scheduledForDate = useMemo2(() => {
    ...
    const todayCheckedCount = useMemo2(() => {
    ...
    const availRoster = useMemo2(
      () => sortedAZ.filter((s) => !dayIds.includes(s.id)),
      [sortedAZ, dayIds]
    );
    const analytics = useMemo2(() => {
    ...
    const allLogs = useMemo2(() => auditAsLogs.length ? [...db.logs, ...auditAsLogs].sort((a, b) => logTimestamp(b) - logTimestamp(a)) : db.logs, [db.logs, auditAsLogs]);
    ...
    const logActions = useMemo2(() => [...new Set(allLogs.map((l) => l.action))].sort(), [allLogs]);
    useEffect2(() => {
      setLPage(1);
...
已经排过的日期、已签到的记录和学员课时都不受影响。`, async () => {
    ...
    const teachableMembers = useMemo2(
...
提醒：您的上课时间是 ${fmtDate(rDate)}${slot ? ` ${slot}` : ""}，请准时到课。${tenantDisplayName} 期待见到您！`;
    ...
    const batchCheckIn = () => {
      ...
      if (!elig.length) {
        showToast(already ? "今日排课学员均已签到 ✓" : "今日无可签到/消课学员", "warn");
        return;
      }
...
已经用它排过的课、学员档案和课时都不受影响。`, async () => {
    ...
    const openGrowthReport = (s) => {
      ...
      const shareMsg = isNew ? `欢迎 ${s.name} 加入 ${reportStudioName}！学习旅程刚刚启程，期待记录每一份成长与快乐。` : `${s.name} 在 ${reportStudioName} 已经学习了 ${days} 天，累计上课 ${checkins.length} 次，完成${workNoun} ${port.length} 份！每一次练习都是成长的印记，期待继续陪伴 TA 自信探索。`;
...
照片会从服务器删除，家长在学员专区也将不再看到它。如果只是不想公开展示，取消勾选「展示到官网作品墙」即可，照片仍会保留在私人记录里。`, async () => {
    ...
    return /* @__PURE__ */ React.createElement("div", { className: "flex h-screen bg-gray-50" }, toast && /* @__PURE__ */ React.createElement(Toast, { key: toast.key, msg: toast.msg, type: toast.type, action: toast.action, onDone: () => setToast(null) }), icsPreview && /* @__PURE__ */ React.createElement(
      ...
      "div",
      {
        ...
        className: tab === "settings" ? "fixed inset-0 bg-gray-50 z-[60] overflow-y-auto" : "fixed inset-0 bg-black/60 z-[60] flex items-center justify-center p-4",
      ...
      /* @__PURE__ */ React.createElement(
        "div",
        {
          className: tab === "settings" ? "min-h-full w-full max-w-6xl mx-auto px-4 py-5 md:px-8 md:py-8" : "bg-white rounded-2xl p-6 w-full max-w-2xl shadow-2xl anim overflow-y-auto modal-scroll",
          style: tab === "settings" ? void 0 : { maxHeight: "90dvh" },
          onClick: (e) => e.stopPropagation()
        },
        /* @__PURE__ */ React.createElement("div", { className: "flex justify-between items-center mb-5" }, /* @__PURE__ */ React.createElement("div", null, tab === "settings" && /* @__PURE__ */ React.createElement("button", { type: "button", onClick: closeSettings, className: "inline-flex items-center gap-1 text-xs font-bold text-indigo-600 mb-1 min-h-[32px]" }, /* @__PURE__ */ React.createElement(Icon, { name: "chevronLeft", className: "w-4 h-4" }), "返回工作台"), /* @__PURE__ */ React.createElement("h3", { id: "settings-dialog-title", className: "inline-flex items-center gap-1.5 font-bold text-gray-800 text-xl" }, /* @__PURE__ */ React.createElement(Icon, { name: "cog", className: "w-5 h-5" }), "系统设置")), /* @__PURE__ */ React.createElement("button", { onClick: closeSettings, "aria-label": tab === "settings" ? "返回工作台" : "关闭", className: "text-gray-400 active:text-gray-700 text-xl p-1 min-h-[44px] min-w-[44px] inline-flex items-center justify-center" }, "×")),
        tab === "settings" && /* @__PURE__ */ React.createElement("div", { className: "mb-6 rounded-2xl border border-indigo-100 bg-white p-2 shadow-sm" }, /* @__PURE__ */ React.createElement("div", { className: "flex gap-1 overflow-x-auto", role: "tablist", "aria-label": "系统设置分区" }, [["account", "账号与安全"], ["team", "团队与权限"], ["operational", "运营默认"], ["maintenance", "数据维护"], ["workspace", "工作区链接"]].map(([key, label]) => /* @__PURE__ */ React.createElement(
          "button",
          {
            key,
            type: "button",
            ...
            className: `whitespace-nowrap min-h-[44px] px-3 rounded-xl text-xs font-bold ${settingsSection === key ? "bg-indigo-600 text-white" : "text-gray-600 hover:bg-indigo-50"}`
          },
          label
        )))),
        ...
        /* @__PURE__ */ React.createElement("div", { id: "settings-account", className: "space-y-2 scroll-mt-24" }, /* @__PURE__ */ React.createElement("p", { className: "text-xs font-bold text-gray-500 uppercase tracking-wide" }, "修改登录密码"), /* @__PURE__ */ React.createElement("label", { className: "block text-xs font-bold text-gray-600" }, "当前密码", /* @__PURE__ */ React.createElement(
          "input",
          {
            type: "password",
...
此操作不影响任何统计数据。`, async () => {
      ...
      tab === "dashboard" && /* @__PURE__ */ React.createElement("div", { className: "cms-dashboard-root anim space-y-5" }, /* @__PURE__ */ React.createElement("h2", { className: "inline-flex items-center gap-1.5 text-xl md:text-2xl font-bold text-gray-800" }, /* @__PURE__ */ React.createElement(Icon, { name: "dashboard", className: "w-4 h-4" }), "工作台"), (() => {
        const actionsByRole = {
        ...
        ")"
      )))), analytics.lowBalance.length > 0 && /* @__PURE__ */ React.createElement("div", { className: "bg-amber-50 border border-amber-200 rounded-2xl p-4" }, /* @__PURE__ */ React.createElement("p", { className: "inline-flex items-center gap-1.5 font-bold text-amber-800 mb-2 text-sm" }, /* @__PURE__ */ React.createElement(Icon, { name: "bolt", className: "w-4 h-4" }), "课时预警 — ", analytics.lowBalance.length, " 名学员余额 ≤ 2 课时"), /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap gap-2" }, analytics.lowBalance.map((s) => /* @__PURE__ */ React.createElement(
        "button",
        {
          key: s.id,
          onClick: () => {
      ...
      tab === "roster" && /* @__PURE__ */ React.createElement("div", { className: "anim space-y-4" }, /* @__PURE__ */ React.createElement("h2", { className: "inline-flex items-center gap-1.5 text-xl md:text-2xl font-bold text-gray-800" }, /* @__PURE__ */ React.createElement(Icon, { name: "calendar", className: "w-4 h-4" }), "课程安排"), scheduleLoadError && /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-3 rounded-2xl border border-red-200 bg-red-50 p-3 text-sm text-red-700" }, /* @__PURE__ */ React.createElement("span", { className: "flex-1" }, scheduleLoadError), /* @__PURE__ */ React.createElement("button", { onClick: loadSchedules, className: "rounded-xl border border-red-200 bg-white px-3 py-2 text-xs font-bold min-h-[44px]" }, "重试")), upcomingBirthdays.length > 0 && /* @__PURE__ */ React.createElement("details", { className: "bg-pink-50 border border-pink-200 rounded-2xl overflow-hidden group" }, /* @__PURE__ */ React.createElement("summary", { className: "list-none cursor-pointer min-h-[44px] px-4 py-2 flex items-center justify-between gap-3 text-sm font-bold text-rose-700" }, /* @__PURE__ */ React.createElement("span", { className: "inline-flex items-center gap-1.5" }, /* @__PURE__ */ React.createElement(Icon, { name: "cake", className: "w-4 h-4" }), "近 14 天生日（", upcomingBirthdays.length, " 人）"), /* @__PURE__ */ React.createElement("span", { className: "group-open:rotate-180 transition-transform", "aria-hidden": "true" }, "⌄")), /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap gap-2 px-4 pb-4 border-t border-pink-200 pt-3" }, upcomingBirthdays.map(({ s, in: days, md, age }) => /* @__PURE__ */ React.createElement("button", { key: s.id, onClick: () => {
      ...
      }, className: "bg-white border border-pink-200 active:bg-pink-50 rounded-xl px-3 py-2 text-left" }, /* @__PURE__ */ React.createElement("p", { className: "text-sm font-bold text-gray-800" }, s.name, " ", /* @__PURE__ */ React.createElement("span", { className: "text-xs font-normal text-rose-400" }, days === 0 ? "今天" : `${md} (${days}天后)`)), /* @__PURE__ */ React.createElement("p", { className: "text-[11px] text-gray-400" }, "点击复制生日祝福话术"))))), TENANT_SLUG && /* @__PURE__ */ React.createElement("details", { className: "bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden group" }, /* @__PURE__ */ React.createElement("summary", { className: "list-none cursor-pointer min-h-[52px] px-4 py-3 flex items-center justify-between gap-3" }, /* @__PURE__ */ React.createElement("span", { className: "inline-flex items-center gap-2 min-w-0" }, /* @__PURE__ */ React.createElement(Icon, { name: "calendar", className: "w-4 h-4 text-gray-500" }), /* @__PURE__ */ React.createElement("span", null, /* @__PURE__ */ React.createElement("span", { className: "block text-sm font-bold text-gray-800" }, "固定课表"), /* @__PURE__ */ React.createElement("span", { className: "block text-xs font-normal text-gray-400" }, schedules.length ? `${schedules.length} 个每周班次` : "创建每周自动排课班次"))), /* @__PURE__ */ React.createElement("span", { className: "text-indigo-600 group-open:rotate-180 transition-transform", "aria-hidden": "true" }, "⌄")), /* @__PURE__ */ React.createElement("div", { className: "p-4 space-y-3 border-t border-gray-100" }, /* @__PURE__ */ React.createElement("div", { className: "flex justify-between items-center gap-2 flex-wrap" }, /* @__PURE__ */ React.createElement("p", { className: "inline-flex items-center gap-1.5 font-bold text-sm text-gray-800" }, /* @__PURE__ */ React.createElement(Icon, { name: "calendar", className: "w-4 h-4" }), "每周课表 ", /* @__PURE__ */ React.createElement("span", { className: "text-xs font-normal text-gray-400" }, "固定班次按周几自动排入当日名单")), /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-2 flex-wrap" }, /* @__PURE__ */ React.createElement(
        "button",
        {
      ...
      })(), /* @__PURE__ */ React.createElement("div", { className: "cms-roster-add border-t border-gray-100 pt-3", id: "rosterAddStudent" }, /* @__PURE__ */ React.createElement("div", { className: "cms-roster-add-fields" }, /* @__PURE__ */ React.createElement("div", { className: "min-w-0" }, /* @__PURE__ */ React.createElement("label", { className: "text-xs font-bold text-gray-500 mb-1 block" }, "添加学员"), /* @__PURE__ */ React.createElement(StudentPicker, { students: availRoster, value: rPick, onChange: setRPick, placeholder: "搜索并选择学员..." })), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("label", { className: "text-xs font-bold text-gray-500 mb-1 block" }, "上课时间"), /* @__PURE__ */ React.createElement(
        "input",
        {
          type: "time",
          value: rTime,
        ...
        rPick ? "加入课程安排" : "请先选择学员"
      )), /* @__PURE__ */ React.createElement("label", { className: "inline-flex items-center gap-2 mt-2 text-xs font-bold text-gray-500 min-h-[44px]" }, /* @__PURE__ */ React.createElement("input", { type: "checkbox", checked: rOneToOne, onChange: (e) => setROneToOne(e.target.checked), className: "w-4 h-4" }), "1 对 1（同时段还有其他人时会提示冲突）")), /* @__PURE__ */ React.createElement("details", { className: "pt-3 border-t border-gray-100 group" }, /* @__PURE__ */ React.createElement("summary", { className: "list-none cursor-pointer min-h-[44px] flex items-center justify-between gap-3 text-xs font-bold text-gray-600" }, /* @__PURE__ */ React.createElement("span", { className: "inline-flex items-center gap-1.5" }, /* @__PURE__ */ React.createElement(Icon, { name: "clipboard", className: "w-4 h-4" }), "班组模板与批量工具"), /* @__PURE__ */ React.createElement("span", { className: "text-indigo-600 group-open:rotate-180 transition-transform", "aria-hidden": "true" }, "⌄")), /* @__PURE__ */ React.createElement("div", { className: "pt-2 flex gap-2 items-center flex-wrap" }, /* @__PURE__ */ React.createElement(
        "select",
        {
          value: grpSel,
        ...
        const s = db.students.find((x) => x.id === id);
        return s && !s.archived;
      }).length, " 人", scheduledForDate.length > 0 && /* @__PURE__ */ React.createElement("span", { className: "text-xs font-normal text-indigo-500 ml-1" }, "（课表 ", scheduledForDate.length, " 班）")), dayIds.length > 0 && /* @__PURE__ */ React.createElement("details", { className: "cms-day-actions-mobile" }, /* @__PURE__ */ React.createElement("summary", null, /* @__PURE__ */ React.createElement(Icon, { name: "ellipsis", className: "w-4 h-4" }), "当日操作"), /* @__PURE__ */ React.createElement("div", { className: "cms-roster-menu" }, canExportData && /* @__PURE__ */ React.createElement("button", { onClick: () => openIcsPreview("roster"), disabled: icsBusy }, /* @__PURE__ */ React.createElement(Icon, { name: "calendar", className: "w-4 h-4" }), "导出当日 ICS"), /* @__PURE__ */ React.createElement("button", { onClick: copyRosterDaily }, /* @__PURE__ */ React.createElement(Icon, { name: "clipboard", className: "w-4 h-4" }), "复制日报"), dayIds.some((id) => {
        const s = db.students.find((x) => x.id === id);
        return s && !s.archived && s.mobile;
      }) && /* @__PURE__ */ React.createElement("button", { onClick: copyRosterReminders }, /* @__PURE__ */ React.createElement(Icon, { name: "chat", className: "w-4 h-4" }), "批量提醒"), dayIds.some((id) => {
        const s = db.students.find((x) => x.id === id);
        return s && !s.archived && s.balance > 0;
      }) && /* @__PURE__ */ React.createElement("button", { onClick: batchCheckIn, disabled: busy }, /* @__PURE__ */ React.createElement(Icon, { name: "check", className: "w-4 h-4" }), "批量签到并扣课时"))), /* @__PURE__ */ React.createElement("div", { className: "cms-day-actions-desktop flex gap-2 flex-wrap" }, dayIds.length > 0 && canExportData && /* @__PURE__ */ React.createElement(
        "button",
        {
        ...
        const s = db.students.find((x) => x.id === id);
        return s && !s.archived && s.mobile;
      }) && /* @__PURE__ */ React.createElement("button", { onClick: copyRosterReminders, className: "bg-white border border-green-300 active:bg-green-50 text-green-700 px-3 py-1.5 rounded-xl text-xs font-bold min-h-[44px]" }, /* @__PURE__ */ React.createElement("span", { className: "inline-flex items-center gap-1.5" }, /* @__PURE__ */ React.createElement(Icon, { name: "chat", className: "w-4 h-4" }), "批量提醒")), dayIds.some((id) => {
        const s = db.students.find((x) => x.id === id);
        return s && !s.archived && s.balance > 0;
      }) && /* @__PURE__ */ React.createElement(
        "button",
        ...
        return /* @__PURE__ */ React.createElement("div", { key: sid, className: `cms-roster-row hover-row ${lowBal ? "is-low" : ""}` }, /* @__PURE__ */ React.createElement("div", { className: "cms-roster-info" }, /* @__PURE__ */ React.createElement(PhotoAvatar, { photo: s.photo, name: s.name, size: "sm" }), /* @__PURE__ */ React.createElement("div", { className: "flex-1 min-w-0" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-2 min-w-0 flex-wrap" }, /* @__PURE__ */ React.createElement("p", { className: "font-bold text-gray-900 truncate" }, s.name), entry.oneToOne && /* @__PURE__ */ React.createElement("span", { className: "text-[11px] font-bold text-indigo-600 bg-indigo-50 border border-indigo-200 rounded-full px-2 py-0.5" }, "1 对 1"), /* @__PURE__ */ React.createElement("span", { className: `text-[11px] font-bold rounded-full px-2 py-0.5 border ${isDone ? "bg-green-50 border-green-200 text-green-700" : entry.status === "makeup" ? "bg-blue-50 border-blue-200 text-blue-700" : "bg-gray-50 border-gray-200 text-gray-600"}` }, rosterStatus)), /* @__PURE__ */ React.createElement("p", { className: "text-xs text-gray-400 truncate" }, [s.mobile || "未填写手机", slot, entry.note].filter(Boolean).join(" · "))), /* @__PURE__ */ React.createElement(BalBadge, { n: s.balance })), /* @__PURE__ */ React.createElement("div", { className: `cms-roster-actions ${lowBal ? "has-reminder" : ""}` }, TENANT_SLUG && entry.id && /* @__PURE__ */ React.createElement(
        ...
        ), TENANT_SLUG && !entry.id && /* @__PURE__ */ React.createElement("span", { className: "cms-roster-time px-3 py-2 border border-gray-200 rounded-xl bg-gray-50 text-xs font-bold text-gray-700 min-h-[44px] inline-flex items-center" }, slot || "未设时间"), lowBal && /* @__PURE__ */ React.createElement("button", { onClick: () => {
          const msg = renderMessage(
            "renewal",
            "{student} 家长您好！温馨提醒：您在 {studio} 的剩余课时为 {balance} 节{note}，为不影响后续上课安排，欢迎随时联系老师续课。",
            { student: s.name, balance: s.balance, note: (parseInt(s.balance, 10) || 0) === 0 ? "（已用完）" : "" }
        ...
        }, className: "cms-roster-reminder" }, /* @__PURE__ */ React.createElement(Icon, { name: "chat", className: "w-4 h-4" }), "续费提醒"), isDone ? /* @__PURE__ */ React.createElement("button", { disabled: true, className: "cms-roster-primary is-done" }, /* @__PURE__ */ React.createElement(Icon, { name: "check", className: "w-4 h-4" }), "已签到") : /* @__PURE__ */ React.createElement(
          "button",
          {
          ...
          s.balance > 0 ? "签到并扣 1 课时" : "余额不足"
        ), /* @__PURE__ */ React.createElement("details", { className: "cms-roster-more", name: "roster-student-actions" }, /* @__PURE__ */ React.createElement("summary", { "aria-label": `${s.name} 更多操作` }, /* @__PURE__ */ React.createElement(Icon, { name: "ellipsis", className: "w-5 h-5" })), /* @__PURE__ */ React.createElement("div", { className: "cms-roster-menu" }, /* @__PURE__ */ React.createElement("div", { className: "cms-roster-menu__context" }, /* @__PURE__ */ React.createElement("strong", null, s.name), /* @__PURE__ */ React.createElement("span", null, fmtDate(rDate), " · ", slot || "时间未设置", " · 余额 ", s.balance)), entry.id && /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("p", { className: "cms-roster-menu__label" }, "课程状态"), /* @__PURE__ */ React.createElement("button", { onClick: (e) => {
          ...
          e.currentTarget.closest("details")?.removeAttribute("open");
        }, disabled: busy }, /* @__PURE__ */ React.createElement(Icon, { name: "users", className: "w-4 h-4" }), entry.oneToOne ? "改为普通班课" : "标记为 1 对 1"), isDone && /* @__PURE__ */ React.createElement("button", { onClick: (e) => {
          undoCheckIn(s.id, s.name);
          e.currentTarget.closest("details")?.removeAttribute("open");
        }, disabled: busy }, /* @__PURE__ */ React.createElement(Icon, { name: "refresh", className: "w-4 h-4" }), "撤销本日签到"), entry.id ? /* @__PURE__ */ React.createElement("button", { onClick: (e) => {
          removeFromRoster(s.id);
          e.currentTarget.closest("details")?.removeAttribute("open");
        }, disabled: busy, className: "is-danger" }, /* @__PURE__ */ React.createElement(Icon, { name: "trash", className: "w-4 h-4" }), "移出本日课程安排") : /* @__PURE__ */ React.createElement("p", { className: "cms-roster-menu__source" }, /* @__PURE__ */ React.createElement(Icon, { name: "calendar", className: "w-4 h-4" }), "来自固定课表，需在上方班次中调整")))));
      })))),
      tab === "works" && /* @__PURE__ */ React.createElement("div", { className: "anim space-y-5 max-w-6xl mx-auto" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-start justify-between gap-3 flex-wrap" }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("h2", { className: "inline-flex items-center gap-2 text-xl md:text-2xl font-bold text-gray-800" }, /* @__PURE__ */ React.createElement(Icon, { name: "image", className: "w-5 h-5" }), "作品管理"), /* @__PURE__ */ React.createElement("p", { className: "text-sm text-gray-500 mt-1" }, "从这里按学员浏览作品；具体上传、编辑和公开授权仍在学员档案中完成。")), /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => setTab("students"), className: "min-h-[44px] px-4 rounded-xl border border-indigo-200 bg-indigo-50 text-indigo-700 text-sm font-bold" }, "进入学员档案 →")), /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-2 md:grid-cols-4 gap-3" }, [["作品总数", portfolioEntries.length, "text-gray-900"], ["已公开", portfolioEntries.filter(({ item }) => item.public || item.visibility === "shared").length, "text-emerald-700"], ["待授权", portfolioEntries.filter(({ student }) => student.publicationConsent?.status !== "confirmed").length, "text-amber-700"], ["有作品学员", new Set(portfolioEntries.map(({ student }) => student.id)).size, "text-indigo-700"]].map(([label, value, color]) => /* @__PURE__ */ React.createElement("div", { key: label, className: "bg-white rounded-2xl border border-gray-100 shadow-sm p-4" }, /* @__PURE__ */ React.createElement("p", { className: "text-xs text-gray-400" }, label), /* @__PURE__ */ React.createElement("p", { className: `text-2xl font-bold mt-1 ${color}` }, value)))), /* @__PURE__ */ React.createElement("section", { className: "bg-white rounded-2xl shadow-sm border border-gray-100 p-5", "aria-labelledby": "works-list-title" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-center justify-between gap-3 mb-3" }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("h3", { id: "works-list-title", className: "font-bold text-gray-900" }, "最近作品"), /* @__PURE__ */ React.createElement("p", { className: "text-xs text-gray-400 mt-0.5" }, "按作品日期倒序 · 最多显示最近 50 条")), /* @__PURE__ */ React.createElement("span", { className: "text-xs font-bold text-gray-500" }, portfolioEntries.length, " 条")), !portfolioEntries.length ? /* @__PURE__ */ React.createElement(EmptyState, { icon: /* @__PURE__ */ React.createElement(Icon, { name: "image", className: "w-8 h-8" }), main: "还没有作品", sub: "打开学员档案后，在作品区上传第一件作品。", action: "查看学员", onAction: () => setTab("students") }) : /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3" }, portfolioEntries.slice(0, 50).map(({ student, item }) => {
      ...
      tab === "students" && /* @__PURE__ */ React.createElement("div", { className: "anim space-y-4" }, /* @__PURE__ */ React.createElement("div", { className: "flex justify-between items-center gap-3 flex-wrap" }, /* @__PURE__ */ React.createElement("h2", { className: "text-xl md:text-2xl font-bold text-gray-800" }, `学员档案 (${sortedFiltered.length})`), /* @__PURE__ */ React.createElement("div", { className: "flex gap-2" }, canManageOperations && /* @__PURE__ */ React.createElement(
        ...
        /* @__PURE__ */ React.createElement(Icon, { name: "plus", className: "w-4 h-4" }),
        "新建"
      ))), /* @__PURE__ */ React.createElement("div", { className: "bg-white rounded-2xl shadow-sm border border-gray-100 p-4 space-y-3" }, /* @__PURE__ */ React.createElement("div", { className: "relative" }, /* @__PURE__ */ React.createElement("span", { className: "absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" }, /* @__PURE__ */ React.createElement(Icon, { name: "search" })), /* @__PURE__ */ React.createElement(
        "input",
        {
          type: "text",
      ...
      )), (filterBy !== "all" || srch) && /* @__PURE__ */ React.createElement(
        "button",
        {
          onClick: () => {
            setFilterBy("all");
          ...
          className: "inline-flex items-center gap-1 px-3 py-2 rounded-xl text-xs font-bold border border-red-200 text-red-500 bg-white active:bg-red-50 min-h-[44px] flex-shrink-0"
        },
        /* @__PURE__ */ React.createElement(Icon, { name: "close", className: "w-3.5 h-3.5" }),
        "清除"
      ...
      tab === "new_student" && /* @__PURE__ */ React.createElement("div", { className: "anim bg-white rounded-2xl p-6 max-w-xl mx-auto shadow-sm border border-gray-100" }, /* @__PURE__ */ React.createElement("h2", { className: "inline-flex items-center gap-2 text-xl md:text-2xl font-bold mb-5 text-gray-800" }, /* @__PURE__ */ React.createElement(Icon, { name: "plus", className: "w-5 h-5" }), "新建学员档案"), /* @__PURE__ */ React.createElement("form", { onSubmit: handleAddStudent, className: "space-y-4" }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("label", { className: "text-sm font-bold text-gray-500 mb-2 block" }, "照片 Photo ", /* @__PURE__ */ React.createElement("span", { className: "font-normal text-gray-400" }, "选填")), /* @__PURE__ */ React.createElement(PhotoUploader, { value: formPhoto, onChange: setFormPhoto, notify })), /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-2 gap-3" }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("label", { className: "text-sm font-bold text-gray-500 mb-1 block" }, "First Name (名) *"), /* @__PURE__ */ React.createElement(
        "input",
        {
          required: true,
          name: "firstName",
      ...
      tab === "pending" && /* @__PURE__ */ React.createElement("div", { className: "anim space-y-4" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-start justify-between gap-3 flex-wrap" }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("h2", { className: "inline-flex items-center gap-1.5 text-xl md:text-2xl font-bold text-gray-800" }, /* @__PURE__ */ React.createElement(Icon, { name: "clipboard", className: "w-4 h-4" }), "待处理"), /* @__PURE__ */ React.createElement("p", { className: "text-sm text-gray-500 mt-1" }, "新报名和约课申请共用一个收件箱，按业务类型分开处理。")), /* @__PURE__ */ React.createElement("span", { className: "rounded-full bg-amber-50 border border-amber-200 px-3 py-1 text-xs font-bold text-amber-700" }, pendingCount, " 项等待处理")), /* @__PURE__ */ React.createElement("div", { className: "flex gap-2 flex-wrap" }, [
      ...
      tab === "topup" && /* @__PURE__ */ React.createElement("div", { className: "anim bg-white rounded-2xl shadow-sm border border-gray-100 p-6 max-w-2xl mx-auto" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-start justify-between gap-3 mb-4" }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("h2", { className: "inline-flex items-center gap-1.5 text-xl md:text-2xl font-bold text-gray-800" }, /* @__PURE__ */ React.createElement(Icon, { name: "money", className: "w-4 h-4" }), "充值与退款"), /* @__PURE__ */ React.createElement("p", { className: "text-sm text-gray-500 mt-1" }, "先选择学员，再完成充值或退款；支付渠道只记录实际收款方式，不在 CMS 内接入在线支付。"))), canManageOperations && /* @__PURE__ */ React.createElement("details", { open: true, className: "mb-5 rounded-2xl border border-indigo-100 bg-indigo-50/60 overflow-hidden" }, /* @__PURE__ */ React.createElement("summary", { className: "cursor-pointer select-none px-4 py-3 min-h-[48px] inline-flex items-center gap-2 text-sm font-bold text-indigo-900" }, /* @__PURE__ */ React.createElement(Icon, { name: "card", className: "w-4 h-4" }), "套餐管理 ", /* @__PURE__ */ React.createElement("span", { className: "text-xs font-normal text-indigo-500" }, (db.packages || []).length, " 个")), /* @__PURE__ */ React.createElement("div", { className: "p-4 pt-1 space-y-3" }, /* @__PURE__ */ React.createElement("p", { className: "text-xs text-indigo-700 leading-relaxed" }, "这里定义前台充值时可快速选择的课包。修改套餐不会改动历史充值记录；删除前请确认它不再需要被新收款使用。"), (db.packages || []).map((pkg) => /* @__PURE__ */ React.createElement("div", { key: pkg.id, className: "flex items-center gap-3 rounded-xl border border-indigo-100 bg-white px-3 py-2.5" }, /* @__PURE__ */ React.createElement("div", { className: "min-w-0 flex-1" }, /* @__PURE__ */ React.createElement("p", { className: "text-sm font-bold text-gray-800 truncate" }, pkg.name), /* @__PURE__ */ React.createElement("p", { className: "text-xs text-gray-500 mt-0.5" }, pkg.credits, " 课时 · AUD ", Number(pkg.price || 0).toFixed(2))), /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => {
        setPkgEditId(pkg.id);
        setPkgName(pkg.name);
      ...
      tab === "logs" && /* @__PURE__ */ React.createElement("div", { className: "anim space-y-4" }, /* @__PURE__ */ React.createElement("h2", { className: "inline-flex items-center gap-1.5 text-xl md:text-2xl font-bold text-gray-800" }, /* @__PURE__ */ React.createElement(Icon, { name: "scroll", className: "w-4 h-4" }), "操作日志"), /* @__PURE__ */ React.createElement("div", { className: "bg-white rounded-2xl shadow-sm border border-gray-100 p-4 space-y-3" }, /* @__PURE__ */ React.createElement("div", { className: "flex flex-col sm:flex-row gap-3" }, /* @__PURE__ */ React.createElement("div", { className: "flex-1" }, /* @__PURE__ */ React.createElement(StudentPicker, { students: sortedAZ, value: lStu, onChange: setLStu, placeholder: "精确筛选学员…", showBal: false })), /* @__PURE__ */ React.createElement(
        "select",
        {
          value: lAct,
        ...
        "button",
        {
          ...
          className: "inline-flex items-center gap-1 px-3 py-2 bg-gray-100 active:bg-gray-200 text-gray-500 rounded-xl text-xs font-bold min-h-[44px]"
        },
        /* @__PURE__ */ React.createElement(Icon, { name: "close", className: "w-3.5 h-3.5" }),
        "清除"
      ), /* @__PURE__ */ React.createElement("span", { className: "text-sm text-gray-400" }, filteredLogs.length, " 条"), canManageOperations && /* @__PURE__ */ React.createElement(
        "button",
        {
          onClick: exportLogsCSV,
          className: "inline-flex items-center gap-1.5 ml-auto bg-white border border-gray-300 active:bg-gray-50 text-gray-600 px-3 py-2 rounded-xl font-bold text-xs min-h-[44px]"
        },
        /* @__PURE__ */ React.createElement(Icon, { name: "download", className: "w-4 h-4" }),
        "CSV"
      ...
      selS && /* @__PURE__ */ React.createElement(
        "div",
        {
