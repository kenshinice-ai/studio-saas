...
(function () {
  ...
  const en = Object.fromEntries([
    ...
    ['学员是服务对象；付款方是发票收件人。两条入口最终都选择同一个付款方记录。',
     'Students receive the service; the payer receives the invoice. Both paths resolve to the same payer record.'],
    ['已有学员', 'Existing student'], ['其他个人或机构', 'Other person or organisation'],
    ['搜索并选择学员', 'Search and select a student'],
    ['这个学员还没有付款方', 'This student has no payer yet'],
    ['可以从资料预填创建，但保存前请人工复核付款方姓名和联系方式。',
     'You can prefill a payer from the student record, but review the name and contact details before saving.'],
    ['预填：', 'Prefill: '], ['必须明确选择', 'Choose explicitly'],
    ['已默认选中：', 'Selected by default: '],
    ['有多个付款方时不会自动猜测或合并。', 'Multiple payers are never guessed or merged automatically.'],
    ['先搜索已有付款方', 'Search existing payers first'],
    ['搜索已有付款方', 'Search existing payers'],
    ['新建个人或机构付款方', 'Create a person or organisation payer'],
    ['类型', 'Type'], ['个人', 'Person'], ['机构', 'Organisation'],
    ['个人/家庭（兼容旧类型）', 'Person/family (legacy-compatible)'],
    ['姓名', 'Name'], ['联系人姓名（可选）', 'Contact name (optional)'],
    ['机构名称', 'Organisation name'], ['联系人（可选）', 'Contact (optional)'],
    ['邮箱', 'Email'], ['电话', 'Phone'], ['ABN（可选）', 'ABN (optional)'],
    ['账单地址（可选）', 'Billing address (optional)'], ['付款期限（天）', 'Payment terms (days)'],
    ['PO reference（可选）', 'PO reference (optional)'], ['语言', 'Language'],
    ['可选关联服务对象（0..N）', 'Optional service subjects (0..N)'],
    ['项目说明', 'Item description'], ['数量', 'Quantity'],
    ['单价（未税）', 'Unit price (ex tax)'], ['单价（含税前）', 'Unit price (ex tax)'], ['税率', 'Tax rate'],
    ...
    ['备注（选填）', 'Notes (optional)'], ['合计约', 'Approx. total'], ['（含税）', '(incl. GST)'],
    ['先存成草稿。复核无误后在列表里开具 —— 开具会定号码和到期日，之后金额不能再改。',
     'Save as a draft first. Review it, then issue it from the list — issuing assigns the number and due date and freezes the amount.'],
    ...
    ['发票汇总 CSV', 'Invoice summary CSV'], ['行项目 CSV', 'Invoice lines CSV'],
    ['会计导出不含草稿 · 当前筛选范围', 'Accounting export excludes drafts · current filters'],
    ['发票汇总 CSV 已开始下载', 'Invoice summary CSV download started'],
    ['发票行项目 CSV 已开始下载', 'Invoice lines CSV download started'],
    ...
    /* ── Xero preview boundary ── */
    ['Xero 预接入（Preview）', 'Xero pre-integration (Preview)'],
    ['预览状态 · 不发送数据', 'Preview · No data is sent'],
    ['推送已开启', 'Push enabled'], ['推送未开启', 'Push not enabled'],
    ['历史记录：上次推送', 'History: last push'],
    ['Xero 预接入说明', 'Xero pre-integration notes'],
    ['当前版本只展示接入准备状态，不会向 Xero 发送任何数据。',
     'This version only shows integration readiness; it does not send data to Xero.'],
    ['映射、连接与 gate 状态会保留，真实 transport 上线后再开放生产操作。',
     'Mappings, connection and gate status are retained; production actions open only when the real transport is live.'],
    ['这里可以查看已有的映射与 gate 准备状态；当前版本不会向 Xero 发送任何数据，也不会创建新的推送任务。',
     'You can review existing mappings and gate readiness here; this version sends no data to Xero and creates no new push jobs.'],
    ['已记录连接状态（只读）', 'Connection status recorded (read-only)'],
    ['当前版本不发起 OAuth 连接', 'This version does not start an OAuth connection'],
    ['预览版只显示准备状态，不会发起试跑', 'Preview only shows readiness; it does not run a demo cycle'],
    ['预览阶段只读显示，不修改 gate。', 'Preview is read-only and does not change the gate.'],
    ['Xero 推送', 'Xero push'],
    ['当前版本尚未开放生产推送', 'Production push is not available in this version'],
    ['Xero transport 尚未上线；不会向 Xero 发送任何数据。',
     'The Xero transport is not live; no data is sent to Xero.'],
    ['已有映射、ID 对应表与错误队列仍可查看；真实 transport 上线后再开放推送。',
     'Existing mappings, ID links and the error queue remain viewable; push opens when the real transport is live.'],
    ['预接入阶段只保留已有历史记录与映射状态；不会创建新的 Xero 推送任务。',
     'During pre-integration, existing history and mappings remain available; no new Xero push jobs are created.'],
    ['当前版本尚未接入 Xero transport', 'The Xero transport is not available in this version'],
