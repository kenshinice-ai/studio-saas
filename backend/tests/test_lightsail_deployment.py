...
def test_lightsail_single_node_preserves_roles_backups_and_volumes() -> None:
    ...
    # v10.11.1: assembled the same way, but handed over through the environment
    # rather than on the command line, where /proc/<pid>/cmdline exposed it to
    # every local account. See test_backup_credential_path.py.
    assert control.count("-e STUDIOSAAS_DATABASE_URL \\") == 2
    control_directives = "\n".join(
        line for line in control.splitlines() if not line.lstrip().startswith("#")
    )
    assert "-e STUDIOSAAS_DATABASE_URL=" not in control_directives
