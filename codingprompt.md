...
- 状态/可见性枚举跨层不一致（P0-07）
- `attendance_sessions` 表存在但 api_v1.py 零引用——考勤闭环未实现（P1-05）
...
目前只有 legacy 上传路径。实现 `POST /s/<slug>/v1/media/upload`：扩展名 + MIME + magic byte + 大小校验、路径穿越防护、租户配额、服务端生成 storage_key、租户作用域读取。校验逻辑集中为一个 media service 模块，legacy 上传路径改为调用同一模块。保留 `storage_provider` 的 S3 扩展点。

## P1-04 — 注册审核 → 学生转化闭环（架构图流程 5a）

`registrations.status` 已有 `pending/approved/rejected/duplicate/contacted/archived`。补全 Studio Admin 工作流：待审队列醒目、一键 Approve 创建 student 并回写 registration 关联、Reject/Archive 带原因、转化动作写 audit_logs。

## P1-05 — 课时闭环 + 考勤（架构图流程 5b）

`attendance_sessions` 表存在但 **api_v1.py 零引用**。实现：购买套餐 → `purchase` 交易加课时 → 上课记录 attendance session 并产生 `consume` 交易 → 余额不足保护。credit_transactions 保持账本式（只追加、可追溯到套餐/考勤来源）。
...
`backend/backups/` 目前是空目录。实现 pg_dump 备份脚本 + 恢复步骤 runbook（更新 docs/Admin_Guide.md §Backup），并实际演练一次恢复。
