# PWE Studio Edition v10.12.2 candidate — 交付工程师 Runbook（未发布）
...
1. 本候选尚未形成可交付包；不得向客户发送或部署。发布授权后只使用
   `PWE-Studio-Edition-10.12.2.tar.gz`，同时向客户提供发布方单独保存的 SHA-256。
2. 验证 `BUILD_INFO`：

   ```bash
   grep -E '^(version|mode|commit)=' BUILD_INFO
   # 必须包含 version=10.12.2、mode=standalone
   ```
...
- [ ] 明确签字：v10.12.2 candidate 标准安装只承诺 PostgreSQL 本地备份；媒体文件异地
...
- 媒体文件卷的自动备份与异地副本：暂不纳入 v10.12.2 candidate 标准验收。
