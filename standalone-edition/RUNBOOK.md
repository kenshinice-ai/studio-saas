# PWE Studio Edition v8.0.1 — 交付工程师 Runbook
...
1. 只使用官方 `PWE-Studio-Edition-8.0.1.tar.gz`，同时向客户提供发布方
   单独保存的 SHA-256。
2. 验证 `BUILD_INFO`：

   ```bash
   grep -E '^(version|mode|commit)=' BUILD_INFO
   # 必须包含 version=8.0.1、mode=standalone
   ```
...
- [ ] 明确签字：v8.0.1 只承诺 PostgreSQL 备份；媒体文件备份暂缓。
...
- 媒体文件卷的自动备份与异地副本：暂不纳入 v8.0.1 验收。
