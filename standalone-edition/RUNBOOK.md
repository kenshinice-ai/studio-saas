# PWE Studio Edition v7.7.8 — 交付工程师 Runbook
...
1. 只使用官方 `PWE-Studio-Edition-7.7.8.tar.gz`，同时向客户提供发布方
   单独保存的 SHA-256。
2. 验证 `BUILD_INFO`：

   ```bash
   grep -E '^(version|mode|commit)=' BUILD_INFO
   # 必须包含 version=7.7.8、mode=standalone
   ```
...
- [ ] 明确签字：v7.7.8 只承诺 PostgreSQL 备份；媒体文件备份暂缓。
...
- 媒体文件卷的自动备份与异地副本：暂不纳入 v7.7.8 验收。
