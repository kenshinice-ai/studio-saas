...
DIST = ROOT / "01 BRAND ASSETS" / "pwe-studio"

# ---------------------------------------------------------------------------
# Palette (single source of truth — mirrored in docs/design/Brand_Identity.md)
# ---------------------------------------------------------------------------
NAVY = "#0E1729"          # Paradise/PWE family navy — brand ink
INK_ON_DARK = "#FFFFFF"   # pure white ink for dark brand surfaces
AMBER = "#F5B335"         # Paradise/PWE family amber — creative accent
AMBER_ON_DARK = "#F5B335" # same canonical amber on the family navy
CANVAS = "#F7F5F2"        # warm paper used by family marketing surfaces
...
def main():
    ...
    for directory in (DIST / "svg", DIST / "png", DIST / "pwa", DIST / "source"):
        directory.mkdir(parents=True, exist_ok=True)

    svg_outputs = {
        "pwe-mark.svg": mark_light,
        "pwe-mark-dark.svg": mark_dark,
        "pwe-logo.svg": lockup_light,
        "pwe-logo-dark.svg": lockup_dark,
    }
    ...
    (DIST / "pwa" / "favicon.svg").write_text(favicon, encoding="utf-8")
    ...
    logo = render_lockup_png(800, navy + (255,), amber + (255,))
    logo_dark = render_lockup_png(800, white + (255,), amber_d + (255,))
    logo.save(ROOT / "logo.png")
    logo_dark.save(ROOT / "logo-light.png")
    logo.save(DIST / "png" / "pwe-logo-800.png")
    logo_dark.save(DIST / "png" / "pwe-logo-on-navy-800.png")
    ...
    for px, name in ((192, "icon-192.png"), (512, "icon-512.png"),
        ...
        img.convert("RGB").save(DIST / "pwa" / name)

    # Transparent mark exports cover social/avatar and compact-document use.
    for px in (64, 256, 512, 1024):
        render_mark_png(
            px,
            navy + (255,),
            amber + (255,),
            fill_frac=0.80,
        ).save(DIST / "png" / f"pwe-mark-{px}.png")
        render_mark_png(
            px,
            white + (255,),
            amber_d + (255,),
            fill_frac=0.80,
        ).save(DIST / "png" / f"pwe-mark-on-navy-{px}.png")

    # Keep a copy of the deterministic geometry source beside the distribution
    # assets. The repository file remains authoritative; this copy makes the
    # human-facing asset kit self-describing without changing runtime imports.
    (DIST / "source" / "render_assets.py").write_text(
